...
def list_newtodolistitems(request, pk):
    ...
    new_todo_list = NewTodoListForm()
    if request.method == 'POST':
        new_todo_list = NewTodoListForm(request.POST)
...
def display_todolist_as_table(request):
    ...
    return render(request, 'app_todolist/display_todolist_as_table.html', {'my_objects': my_objects})
...
def display_todolist_as_table_id(request, pk):
    ...
    cache_tree_children(my_objects)
    return render(request, 'app_todolist/display_todolist_as_table_id.html', {'my_objects': my_objects})

@login_required(login_url='login')   
def display_tree(request):
    my_objects = NewTodoList.objects.filter(active=True)
    cache_tree_children(my_objects)
    return render(request, 'app_todolist/display_tree.html', {'my_objects': my_objects})
...
def clone_newtodolistitems(request, pk):
    ...
    clone_node = NewTodoList.objects.create(title=node.title, parent=node.parent)
